 ...
 * 4, Router 也有中间件的概念，其执行方式和Skoa 中间件执行方式一致，使用doMiddleware处理中间件， 所以我们需要将doMiddleware 方法抽离出来，形成一个通用的模块，
 * 这个模块的主要功能是来组成中间件的执行，所以我们可以创建一个compose(英文翻译： 构成，组成)的模块，
 * 5，我们会将路由对象存放在Router 实例对象的一个数组属性上，我们可以创建一个Layer 模块，用来维护每个路由的信息
 */
